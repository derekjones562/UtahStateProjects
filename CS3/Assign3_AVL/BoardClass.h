...
public:
	...
	Board(); //inializes history and state and sets board to winning board
	...
	bool ofile(string fname); //opens file "fname" and prints the board to it
	void solve();
...
//
bool Board::ofile(string fname = "test.txt")
{
	ofstream f(fname,ios::out);
	if(f)
	{
		f <<"State "<<state<<" From State "<<history<<" "<<fullHistory<<endl;
		for(int i = 0; i <SIZE; i++)
		{
			for(int j = 0; j <SIZE; j++)
			{
...
void Board::solve()
{
	...
	int popcount=0; //count how many you remove from the q
		...
		while(sID!= -1)
	{
		for(int i=0; i<12; i++)
		{
			...
			{
				cout << "State "<<GameBoard.state<<" From "<<GameBoard.history <<GameBoard.fullHistory<< endl;
				cout<<"You WIN!!!\n";
				sID = -1;
				break;
			}
